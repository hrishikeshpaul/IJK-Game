...
class Game_IJK:
    def __init__(self, game, currentPlayer, deterministic, value=0, dir=''):
        self.__game = game
    ...
    def __generate_moves(self, board, player):
        available = []
        for move in ['L', 'D', 'U', 'R']:
            boardCopy = copy.deepcopy(board)
            newc = boardCopy.makeMove(move)
            # newc.printGame()
            if newc:
                # newc.printGame()
                newnode = Game_IJK(newc, player, True, 0, move)
                available.append(newnode)

        return available

    def __evaluate(self, mat):
        total_merges = 0
    ...
    def generateMoves(self, board, player):
        return self.__generate_moves(board, player)

    def evaluate(self, mat):
        return self.__evaluate(mat)
